...
def saveDF(output_path, df) -> None:
    ...
    plt.savefig(f'{output_path}/{current_time}_market_psychology.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def addPolarizationMetrics(df):
    ...
    df['extremity'] = df.apply(
        lambda row: 1.0 if row['neutral'] == 0 and row['volume'] > 0 
        else 1 / row['neutral'] if row['neutral'] > 0 
        else 0, 
        axis=1
    ).round(4)
    return df
